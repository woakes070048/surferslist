	...
	protected function getMoreData(&$data, $customer_group_id, $cache = true) {
		...
		if (!isset($product_data['page_title'])) {
			...
			$product_data = array_merge($product_data, array(
				...
				'tab_question' 		=> $this->language->get('tab_question'), // sprintf($this->language->get('tab_question'), $data['questions']),
				'text_questions'	=> sprintf($this->language->get('text_questions'), (int)$data['questions']),
				'text_minimum' 		=> sprintf($this->language->get('text_minimum'), $data['minimum']),
