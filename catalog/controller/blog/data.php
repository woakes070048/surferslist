...
class ControllerBlogData extends Controller {
    ...
    protected function getMinData(&$data, $customer_group_id, $cache = true) {
        ...
        if ($article_data === false) {
			$limit_char = !empty($this->config->get('blog_limit_char')) ? $this->config->get('blog_limit_char') : 100;
			...
			$article_data = array(
				...
				'name'              => $data['name'],
				'published'         => date($this->language->get('date_format_short'), strtotime($data['date_available'])),
		...
		if (!isset($article_data['popup'])) {
            ...
            foreach ($article_images as $article_image) {
                if ($article_image['image'] && is_file(DIR_IMAGE . $article_image['image'])) {
                    $image = $this->model_tool_image->resize($article_image['image'], $this->config->get('blog_image_additional_width'), $this->config->get('blog_image_additional_height'), 'fw');
			...
			if ($data['image'] && is_file(DIR_IMAGE . $data['image'])) {
				$image = $this->model_tool_image->resize($data['image'], $this->config->get('blog_image_thumb_width'), $this->config->get('blog_image_thumb_height'), 'fw');
            ...
            foreach ($related_articles as $related_article) {
                ...
                if ($related_info) {
                    ...
                    $desciption = utf8_substr(strip_tags(html_entity_decode($related_info['description'], ENT_QUOTES, 'UTF-8')), 0, $limit_char) . $this->language->get('text_ellipses');
			...
			$article_data = array_merge($article_data, array(
				...
				'description'		=> convert_links(strip_tags_decode($data['description'])),
				...
				'date_modified'     => date($this->language->get('date_format_short'), strtotime($data['date_modified']))
			));
