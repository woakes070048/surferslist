<?php
class ControllerErrorProductUnavailable extends Controller {
	public function index() {
		$this->data = array_merge(
			$this->load->language('product/common'),
			$this->load->language('error/product_unavailable')
		);

		$listing_id = isset($this->request->get['listing_id']) ? (int)$this->request->get['listing_id'] : 0;
		...
		$product_info = $this->model_catalog_product->getProduct($listing_id, true);
