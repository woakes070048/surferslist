	...
	public function index() {
		...
		if (!$redirect) {
			...
			if ($this->cart->hasShipping()) {
				...
				if (!empty($this->session->data['shipping_insurance'])) {
					$data['shipping_insurance'] = $this->session->data['shipping_insurance'];
				} else {
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$product_data[] = array(
					...
					'name'       => $product['name'],
					'model'      => $product['model'],
					'member_customer_id' => (!empty($product['member_customer_id']) ? $product['member_customer_id'] : '0'),
					'image' => $product['image'],
			...
			$this->session->data['order_no'] = $order_no;
			...
			foreach ($this->cart->getProducts() as $product) {
				...
				$this->data['products'][] = array(
					'thumb' => $image,
					'product_id' => $product['product_id'],
					'member_id' => (!empty($product['member_id']) ? $product['member_id'] : '' ),
