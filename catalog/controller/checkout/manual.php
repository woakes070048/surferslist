		...
		if ($this->user->isLogged() && $this->user->hasPermission('modify', 'sale/order')) {
			...
			foreach ($products as $product) {
				...
				$json['order_product'][] = array(
					...
					'member_member' => (!empty($product['member_account_name']) ? $product['member_account_name'] : ''),
					'image_display' => $image,
					'image_file'    => (!empty($product['image']) ? $product['image'] : ''),
					'member_commission' => (!empty($product['member_customer_id']) ? ($product['total'] / 100) * $product['member_commission_rate'] : 0),
					'option'     => $option_data,
			...
			if (isset($this->request->post['from_name']) && isset($this->request->post['from_email']) && isset($this->request->post['to_name']) && isset($this->request->post['to_email']) && isset($this->request->post['amount'])) {
				if (!isset($this->request->post['from_name']) || !$this->validateStringLength($this->request->post['from_name'], 1, 64)) {
					$json['error']['vouchers']['from_name'] = $this->language->get('error_from_name');
				}

				if (!$this->validateEmail($this->request->post['from_email'])) {
					$json['error']['vouchers']['from_email'] = $this->language->get('error_email');
				}

				if (!isset($this->request->post['to_name']) || !$this->validateStringLength($this->request->post['to_name'], 1, 64)) {
					$json['error']['vouchers']['to_name'] = $this->language->get('error_to_name');
				}

				if (!$this->validateEmail($this->request->post['to_email'])) {
					$json['error']['vouchers']['to_email'] = $this->language->get('error_email');
				}

				if (!isset($this->request->post['amount']) || ($this->request->post['amount'] < $this->config->get('config_voucher_min')) || ($this->request->post['amount'] > $this->config->get('config_voucher_max'))) {
