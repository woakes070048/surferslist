...
if (typeof entryOptionValue === 'undefined') {
    var entryOptionValue = 'Option Value';
}
...
function uploadImages(imageId, thumbId, textImageManager) {
	$('#dialog').remove();
...
function uploadFile(download_id) {
...
function addDownload() {
    ...
    uploadFile(product_download_row);
...
(function() {
    ...
    // Accordion
    $('.post-section-container').each(function() {
        $(this).clickOrTouch('.post-section-heading', changePostSection);
    });

    $('.post-section-container').first().find('.post-section-heading').trigger('click');
    ...
    // Tabs
    $('#form.account-product-form #languages a').tabsPost();
    ...
    // brand thumbnail
    $('#form.account-product-form').on('change', 'select[name=\'manufacturer_id\']', function() {
    ...
    $(document).clickOrTouch('.upload-images', function(e) {
    	...
    	uploadImages('image' + imageRowId, 'thumb' + imageRowId, textImageManager);
    });

    // clear image
    $('#form.account-product-form').clickOrTouch('.button_clear', function(e) {
        e.preventDefault();
    ...
    // Related
    $('#form.account-product-form input[name=\'related\']').clickOrTouch(null, function(){
    ...
    // Download
    $('#tab-download #product-download .product-download-row').each(function(index) {
    ...
    $.each($('#tab-download #product-download tbody'), function(index, element) {
        uploadFile(index);
    });

    // Attribute
    $('#tab-attribute #attribute .attribute-row').each(function(index) {
        $(this).clickOrTouch('.button_remove', function(e) {
            e.preventDefault();
    ...
    $('#tab-attribute #attribute tbody').each(function(index, element) {
        attributeAutocomplete(index);
    });

    // Option
    $('#tab-option #vtab-option a').tabsPost();
    ...
    $('#tab-option input[name=\'option\']').catcomplete({
    	minLength: 0,
    	delay: 100,
    	source: function(request, response) {
    		$.ajax({
    	...
    	select: function(event, ui) {
    		...
    		$('#tab-option #option-add').before('<a href="#tab-option-' + option_row + '" id="option-' + option_row + '">' + ui.item.label + '&nbsp;<i class="fa fa-times-circle" alt="" onclick="$(\'#option-' + option_row + '\').remove(); $(\'#tab-option-' + option_row + '\').remove(); $(\'#vtab-option a:first\').trigger(\'click\'); return false;"><i></a>');

    		$('#tab-option #vtab-option a').tabsPost();

    		$('#tab-option #option-' + option_row).trigger('click');

    		$('#tab-option .date').datepicker({dateFormat: 'yy-mm-dd'});

    		// $('#tab-option .datetime').datetimepicker({
    		// 	dateFormat: 'yy-mm-dd',
    		// 	timeFormat: 'h:m'
    		// });
            //
    		// $('#tab-option .time').timepicker({timeFormat: 'h:m'});
    ...
    $('#tab-option input[name=\'option\']').clickOrTouch(null, function(){
        $(this).catcomplete("search");
    });

    $('#tab-option #option-add-button').clickOrTouch(null, function(){
        $('#tab-option input[name=\'option\']').catcomplete("search");
    });
})();
