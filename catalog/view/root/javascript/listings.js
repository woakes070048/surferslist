...
function adjustSidebarPosition(headerHeight, titleHeight) {
    return function(e) {
        ...
        var newSidebarHeight = $('#sidebar').outerHeight();
        ...
        if (windowWidth > 979) {
            ...
            if (scrollTop >= floatStart && scrollTop < floatStop) {
                ...
                $('#sidebar').stop().addClass('sidebar-floating').animate({
                    'top': floatPosition + 'px'
                }, 600, 'swing');
            } else if (scrollTop < floatStart) {
                $('#sidebar').removeClass('sidebar-floating').removeAttr('style');
            }
        }
...
function loadGridItems() {
    return function() {
        ...
        var loadedImages = 0;
        var gridCount = thisContainer.find('.grid-item').length || 0;
        var $progressBar = thisContainer.siblings('.progress-bar').eq(0);
        var $loadMore = thisContainer.siblings('.buttons').find('.load-more');
        var $masonryGrid = thisContainer.masonry({
        ...
        thisContainer.imagesLoaded(function() {
            thisContainer.siblings('.loading').eq(0).fadeOut(200);
        }).always(function () {
            $progressBar.hide();
        ...
        // Load more
        $loadMore.clickOrTouch(null, loadMoreGridItems($loadMore, $masonryGrid));
    }
}

function loadListingItems() {
    if (!$('.listing-items').length) {
        return false;
    }

    var thisContainer = $('.listing-items');
    var $progressBar = thisContainer.siblings('.progress-bar').eq(0);
    var $listingItems = thisContainer.children('.listing-item');
    var totalImages = thisContainer.find('.image').length;
    var loadedImages = 0;
    ...
    thisContainer.imagesLoaded(function() {
        ...
        var wookmark = new Wookmark('.listing-items', {
            autoResize: true,
            fillEmptySpace: true,
            align: 'center',
            container: $('.listing-items-container'),
            direction: 'left',
            resizeDelay: 50,
            offset: 10
        });
...
function loadMoreGridItems(loadMoreButton, masonryGrid) {
    return function(e) {
        e.preventDefault();
        ...
        if (url.length && url.indexOf('/listings') !== -1) {
            ...
            $.ajax({
                ...
                beforeSend: function() {
                    ...
                    loadMoreButton.html('<i class="fa fa-spin fa-circle-o-notch icon-spin"></i> ' + textWait);
                },
                complete: function() {
                    $('.pagination').remove();
                ...
                success: function(json) {
                    ...
                    if (json.listings.length) {
                        ...
                        if (json['more_href']) {
                            ...
                            loadMoreButton.attr('href', decodeURIComponent(json['more_href']));
                            loadMoreButton.html('<i class="fa fa-chevron-down"></i> ' + json['text_more']);
                        } else {
                            loadMoreButton.remove();
                        }
                    } else {
                        loadMoreButton.before('<p><span class="error"><i class="fa fa-info-circle"></i> ' + json['text_none'] + '</span></p>');
                        loadMoreButton.remove();
                    }
                },
                error: handleError
...
$(document).ready(function() {
    ...
    setTimeout(function() {
    	...
    	if ($('#sidebar').length && sidebarHeight < contentHeight) {
    		...
    		$(window).on('scroll resize touchmove', adjustSidebarPosition(headerHeight, titleHeight));
    	}
    }, 1000);

    // Grid Items
    $('.grid').each(loadGridItems());
    ...
    // Profile Filters
    $('#sidebar .profile-filter input[type="checkbox"]').iCheck({
