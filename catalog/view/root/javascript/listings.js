...
// Grid Items (e.g. Listings)
var $grid = $('.grid');

if ($grid.length) {
    $grid.each(function() {
        var $thisGrid = $(this);
        var $gridItems = $thisGrid.find('.grid-item');
        var $loadMore = $thisGrid.siblings('.buttons').find('.load-more');
        var $progressBar = $('.progress-bar');
        var loadedImages = 0; // Counter for loaded images
        var listingCount = $gridItems.length || 0;
        var options = {
        ...
        var $masonryGrid = $thisGrid.masonry(options);
        ...
        .progress(function (instance, image) {
            ...
            $progressBar.css('width', (++loadedImages / listingCount * 100) + '%');
...
$(document).ready(function() {
    // All Categories (and, previously, Profiles and Brands, too)
    var $runbrands = $('#runbrands');

    if ($runbrands.length) {
        var $brands = $('#runbrands li');

        $brands.hide();
        $runbrands.before('<div class="information loading"><p>' + textLoading + '</p><span class="icon"><i class="fa fa-spin fa-circle-o-notch icon-spin"></i></span></div>');

        $runbrands.imagesLoaded(function() {
            $('.loading, .loading-gif').fadeOut(300);

            var options = {
                autoResize: true,
                fillEmptySpace: true,
                align: 'center',
                container: $('#runfiltersbrands'),
                direction: 'left',
                resizeDelay: 50,
                offset: 10
            };

            var wookmark = new Wookmark('#runbrands', options);

            var $filters = $('#filtersbrands li');
