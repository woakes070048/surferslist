...
class ModelCheckoutVoucherTheme extends Model {
	...
	public function getVoucherTheme($voucher_theme_id) {
		$query = $this->db->query("
			SELECT *
			FROM " . DB_PREFIX . "voucher_theme vt
	...
	public function getVoucherThemes($data = array()) {
		if ($data) {
			...
			if ($voucher_theme_data === false) {
				...
				$this->cache->set('voucher_theme.' . (int)$this->config->get('config_language_id'), $voucher_theme_data, $this->cache_expires);
			}
