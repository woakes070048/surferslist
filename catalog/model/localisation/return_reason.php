...
class ModelLocalisationReturnReason extends Model {
	...
	public function addReturnReason($data) {
		foreach ($data['return_reason'] as $language_id => $value) {
			if (isset($return_reason_id)) {
				$this->db->query("
					INSERT INTO " . DB_PREFIX . "return_reason
					SET return_reason_id = '" . (int)$return_reason_id . "'
				...
				$return_reason_id = $this->db->countAffected() ? $this->db->getLastId() : 0;
			}
		}
	...
	public function editReturnReason($return_reason_id, $data) {
		$this->db->query("
			DELETE FROM " . DB_PREFIX . "return_reason
	...
	public function getReturnReasons($data = array()) {
		if ($data) {
			...
			if ($return_reason_data === false) {
				$query = $this->db->query("
					SELECT return_reason_id
					, name FROM " . DB_PREFIX . "return_reason
					WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'
					ORDER BY name
				");
				...
				$this->cache->set('return_reason.' . (int)$this->config->get('config_language_id'), $return_reason_data, $this->cache_expires);
			}
	...
	public function getTotalReturnReasons() {
		$query = $this->db->query("
			SELECT COUNT(return_reason_id) AS total
			FROM " . DB_PREFIX . "return_reason
			WHERE language_id = '" . (int)$this->config->get('config_language_id') . "'
		");
